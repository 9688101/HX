...
import (
	...
	"github.com/9688101/HX/relay/controller"
...
func testChannel(ctx context.Context, channel *entity.Channel, request *relaymodel.GeneralOpenAIRequest) (responseMessage string, err error, openaiErr *relaymodel.Error) {
	startTime := time.Now()
	...
	defer func() {
		...
		go entity.RecordTestLog(ctx, &entity.Log{
...
func testChannels(ctx context.Context, notify bool, scope string) error {
	...
	testAllChannelsLock.Unlock()
	channels, err := entity.GetAllChannels(0, 0, scope)
	if err != nil {
		return err
	}
