...
import (
	...
	"github.com/9688101/HX/common/blacklist"
...
func authHelper(c *gin.Context, minRole int) {
	...
	if username == nil {
		...
		user := entity.ValidateAccessToken(accessToken)
...
func TokenAuth() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		key = parts[0]
		token, err := entity.ValidateUserToken(key)
		if err != nil {
		...
		userEnabled, err := entity.CacheIsUserEnabled(token.UserId)
		if err != nil {
		...
		if len(parts) > 1 {
			if entity.IsAdmin(token.UserId) {
